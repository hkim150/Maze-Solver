...
type generatorFunc func(width, height int) (*maze.Maze, error)

func (f generatorFunc) Generate(width, height int) (*maze.Maze, error) {
	return f(width, height)
}

func Generate(width, height int, algorithm string) (*maze.Maze, error) {
	...
	m, err := genFunc.Generate(width, height)
	if err != nil {
		return m, err
	}
